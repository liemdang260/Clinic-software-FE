...
import { today, dateTimeNow } from "_constants/date";
...
function RequestContent({ data, onClick }) {
  const timeParse = (dateTime) => {
    const milisecond = dayjs() - dayjs(dateTime);
  ...
  return (
    <table className="appoinment-rightbar-content">
      {data.map((d, index) => (
        <tr
          className={d.STATUS_ID === 1 ? "unread" : ""}
          key={`request-today-${index}`}
          onClick={() => onClick(d)}
        >
          <td>{timeParse(d.CREATE_AT)}</td>
          <td>
            <p>{d.PATIENT.PATIENT_NAME}</p>
            <p>{d.PATIENT.PHONE}</p>
          </td>
        </tr>
      ))}
...
function AppointmentDemand() {
  ...
  const dataToday = data.filter((d) => dateTimeNow);
